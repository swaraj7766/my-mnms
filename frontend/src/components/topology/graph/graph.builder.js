...
function buildLinkProps(
  ...
  const className =
    link.className || config.link.className || CONST.LINK_CLASS_NAME;

  const linkFlow = link.linkFlow || config.link.linkFlow || false;

  const strokeDashoffset =
    link.strokeDashoffset || config.link.strokeDashoffset;
...
function buildNodeProps(
  ...
  const highlightFontSize =
    node.highlightFontSize || config.node.highlightFontSize;
